...
// ─────────────────────────── 数据不足提示 ───────────────────────────
//
// 本项目早期版本把「不出现买卖信号」列为硬红线（合规考虑）；
// 该约束已按项目所有者决定移除，`packages/core/src/signal.ts` 现在会直接给出买卖信号。
// 但「数据不足时不要编造」仍然保留 —— 它不是限制，而是保证结论可核对的前提。

/**
 * 交易信号的历史验证披露。
 *
 * 为什么必须显示：回测显示信号在唯一可用的区间内**没有正向区分度**
 * （点估计方向偏负，但重叠校正后不显著）。在结论明确之前，
 * 界面不能假装信号已被验证——那会误导使用者把参考当依据。
 * 结论变化时同步更新这里与 README「回放压测」一节。
 */
export const SIGNAL_BACKTEST_CAVEAT =
  "信号尚无证据支持有效：640 日快照、580 个交易日回放显示，看多与看空信号的前瞻收益差" +
  "仅 +0.16~0.27pp，且分块自助法与稀疏非重叠检验（115/57/28 个独立起点）均跨越 0，" +
  "即与噪声无法区分。请作为观察框架而非交易依据。";
